...
class OverviewResponse(BaseModel):
    completed_revenue: float = Field(ge=0)
    gross_order_value: float = Field(ge=0)
...
class ProductAnalytics(BaseModel):
    ...
    gross_ordered_amount: float = Field(ge=0)
...
class CustomerAnalytics(BaseModel):
    ...
    customer_name: str
    order_count: int = Field(ge=0)
    lifetime_completed_payment_amount: float = Field(ge=0)
    average_order_value: float | None = Field(default=None, ge=0)
