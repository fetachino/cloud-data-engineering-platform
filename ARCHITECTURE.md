...
- `observability/prometheus/`: scrape targets and alert thresholds
- `observability/grafana/`: provisioned datasource and platform dashboard
...
The API exposes `/metrics` with bounded route, method, and status labels. The consumer starts the Prometheus client HTTP server on port 9101 and reports received, processed, duplicate, malformed, failed, and duration metrics. Kafka exporter provides broker/topic/consumer-group visibility, while PostgreSQL exporter provides database availability and activity metrics. Prometheus scrapes these services by Compose service name and Grafana reads the provisioned Prometheus datasource.

Observability is additive: Kafka acknowledgement, database transaction boundaries, and Airflow/dbt execution semantics are unchanged. Airflow remains a local one-shot DAG test workflow, so this milestone does not pretend to monitor a continuously running scheduler.
